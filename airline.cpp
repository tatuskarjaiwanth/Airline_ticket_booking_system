...
string contact_validator(string num)
{
    while (!(num.length() == 10))
    {
...
int fli_num_validation(int num)
{
    ...
    while (!flag)
    {
        ...
        cout << "Enter the correct flight number:";
        cin >> num;
    }
}

char gender_validation(char s)
{
    while (!(s == 'M' || s == 'm' || s == 'F' || s == 'f'))
    {
        cout << "Enter the gender correctly:";
        cin >> s;
    }
    return s;
}
...
void Book_reservation()
{
    ...
    for (int i = 0; i < pas_num; i++)
    {
        ...
        cin >> passenger[i].age;
        passenger[i].age = proper_age(passenger[i].age);

        cout << "Gender(M/F): ";
        cin >> passenger[i].Gender;
        passenger[i].Gender = gender_validation(passenger[i].Gender);

        cout << "Contact No: ";
        cin >> passenger[i].Contact;
        passenger[i].Contact = contact_validator(passenger[i].Contact);

        if(i==0){
        cout << "Flight Number(From the above displayed List): ";
        cin >> passenger[i].flight_number;
        passenger[i].flight_number = fli_num_validation(passenger[i].flight_number);
...
void Cancel_reservation()
{
    ...
    getline(cin , name);
    int num;
    cout<<"Enter flight Number you want to delete present in ticket: ";
    cin>>num;
    num = fli_num_validation(num);
...
void Display_flight_details()
{
    ...
    if(myQueue.empty()){
        cout<<"There are no tickets Booked to delete\n\n";
        return;
    }
